  ...
  return (
    ...
    >
      ...
      <div
        ...
        onDrop={fileDropped}
        tabIndex={0}
        className={`group/fd ${editMode ? "flex" : "hidden"} p-1 ${fileDrag ? "animate-logoExit bg-red-600/30 shadow-xs" : "shadow-sm"} ${pressAnim.includes("uplc") && "scale-95"} hover:text-bw text-bw/80 max-h-[90%] w-[40%] max-w-[40%] overflow-hidden rounded-xl transition-all`}
      >
        <Label className="h-full w-full text-[10px]">
          {fileDrag ? "Drop file" : "Click or drag file"}
          <Input
            ref={inputRef}
            onChange={uploaded}
