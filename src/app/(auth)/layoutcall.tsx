...
export default function LayoutCall({
  ...
  return (
    <div className="z-5 flex h-full max-h-[34rem] w-full max-w-[29.5rem] items-center overflow-hidden rounded-[25px] bg-[#012463] shadow-2xl lg:max-w-[59rem]">
      <div
        className={`${!signUpPath && "lg:translate-x-[100%]"} relative left-0 z-1 h-full w-full flex-col rounded-2xl bg-stone-300 p-2 lg:w-1/2`}
      >
        {" "}
        {children}
      </div>
      ...
      <div
        className={`${!signUpPath && "lg:-translate-x-[100%]"} hidden h-full w-1/2 items-center justify-center bg-[#012463] text-white transition-all duration-600 lg:flex`}
      >
        <Image
          src={signUpImage}
