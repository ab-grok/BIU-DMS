...
export default function LayoutCall({
  ...
  useEffect(() => {
    ...
    } else setIsLoading((p) => p.replace("authLogo,", ""));
  }, [loaded, isLoading]);

  const { theme, setTheme } = useTheme();

  return (
    <div className="border-bw/30 relative z-5 flex h-full max-h-[34rem] w-[98%] max-w-[29.5rem] items-center overflow-hidden rounded-[25px] bg-[#012463] shadow-2xl sm:w-[99%] md:w-[99%] lg:max-w-[59rem]">
      <div
        className={`${authPath && "lg:translate-x-[100%]"} shadow-shadow-bw bg-main-fg relative left-0 z-1 h-full w-full flex-col overflow-hidden rounded-[25px] p-2 shadow-md lg:w-1/2`}
      >
        {" "}
        {children}
      </div>
