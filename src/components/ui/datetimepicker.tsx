...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      className={cn("p-3 backdrop-blur-3xl", className)}
      classNames={{
        months:
    ...
    return (
      <div className="flex h-10 items-center">
        ...
        >
          <SelectTrigger
            ref={ref}
            className="focus:bg-accent focus:text-accent-foreground w-[80px]"
...
>(
  (
    ...
    return (
      <Popover>
        <PopoverTrigger tabIndex={tabIndex} asChild disabled={disabled}>
          <Button
            variant="outline"
            className={cn(
              "from-main-fg/70 text-bw ring-bw/20 w-fit max-w-[10rem] justify-start truncate overflow-hidden bg-gradient-to-b to-slate-800 px-1 text-left text-xs font-normal delay-0 hover:to-stone-500 hover:shadow-sm",
              !displayDate && "text-bw",
        ...
        <PopoverContent onBlur={onBlur} className="w-auto p-0">
          ...
          />
          {granularity !== "day" && (
            <div className="border-border border-t p-3 px-0">
              <TimePicker
                onChange={(value) => {
                  onChange?.(value);
                  setDisplayDate(value);
